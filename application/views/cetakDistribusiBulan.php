...
<head>
  ...
  <title>Cetak | Badan Pertanahan Nasional Kanwil Provinsi Bengkulu</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="row">
          <center><a><img src="<?php echo base_url('assets/silatah/dist/img/logobpn.png'); ?>" width="100" height="100" style=" text-align: center"></a></center>
      </div>
      <div class="row">
        <center>
          <h4>BADAN PERTANAHAN NASIONAL</h4>
          <h5>KANTOR WILAYAH PROVINSI BENGKULU<br></h5>
        <h6>Jl. Basuki Rahmat No. 7, Telepon (0736) 21813, Email bengkulu@bpn.go.id</h6>
        </center>
      </div>
   <hr style="border-style:solid;border-color:black;border-width:8;">
      <div class="row">
        <div class="col-xs-12">
          ...
          <?php if($bulan=='01'){$bln='Januari';}elseif($bulan=='02'){$bln='Februari';}elseif($bulan=='03'){$bln='Maret';}elseif($bulan=='04'){$bln='April';}elseif($bulan=='05'){$bln='Mei';}elseif($bulan=='06'){$bln='Juni';}elseif($bulan=='07'){$bln='Juli';}elseif($bulan=='08'){$bln='Agustus';}elseif($bulan=='09'){$bln='September';}elseif($bulan=='10'){$bln='Oktober';}elseif($bulan=='11'){$bln='November';}elseif($bulan=='12'){$bln='Desember';}?>
                <center><h3 class="box-title">Laporan Distribusi Barang<br><small>Tahun Anggaran&nbsp;:&nbsp;<?php echo $tahun2;?>&nbsp;Bulan&nbsp;:&nbsp;<?php echo $bln;?></h3></center><br>
                <div class="pull-left"><small>
                <p>Per Tanggal&nbsp;: <?php echo tgl_indo(date('Y-m-d')); ?>
                <p>UAKPB&nbsp;: KANWIL BPN PROVINSI BENGKULU
                <p>KODE UAKPB&nbsp;: 056012600432442000KD</p></small>
                </div><br>
                  <table class="table table-bordered" style="border-collapse: collapse; width: 100%; margin: 0 auto;font-size: 10px;">
                    <thead>
                              <tr>
                                <th>No.</th>
                                <th>Kode Barang</th>
                        ...
                        <tr>
                        <td colspan="9" class="text-red">No Bukti : <?php echo $dt['noBukti']?></td>
                        </tr>
                        <?php $where = $this->db->query('select *,tb_kwitansi.jumlah as jlh from tb_kwitansi join tb_barang on tb_kwitansi.kodeBarang=tb_barang.kodeBarang join tb_distribusi on tb_kwitansi.noBukti=tb_distribusi.noBukti join tb_pegawai on tb_kwitansi.NIP_pegawai=tb_pegawai.NIP_pegawai join tb_bidang on tb_pegawai.idBidang=tb_bidang.idBidang where tb_kwitansi.noBukti ="'.$dt['noBukti'].'"')->result_array();
                          foreach($where as $d){?>
                            <tr>
                                      <td><?php echo ++$no;?></td>
                                      <td><?php echo $d['kodeBarang'];?></td>
                                      <td><?php echo $d['namaBarang'];?></td>
                                      <td><?php echo $d['jlh'];?></td>
                                      <td><?php echo $d['tglDistribusi'];?></td>
                                      <td><?php echo $d['namaPegawai'];?></td>
                                      <td><?php echo $d['namaBidang'];?></td>
                                      <?php $nm = $this->db->query('select *,tb_pegawai.namaPegawai as nm from tb_pegawai join tb_user on tb_pegawai.NIP_pegawai=tb_user.NIP_pegawai where tb_user.username = "'.$dt['username'].'"')->result_array(); 
                                      foreach($nm as $nma){?>
                                      <td><?php echo $nma['nm'];?></td>
                                      <?php } ?>
                                      <td><?php $he = $d['jlh'] * $d['hargaSatuan']; echo 'Rp. ' . number_format( $he, 0 , '' , '.' ) . ',-'?></td>
                            </tr>
                        <?php }}?>
                          <tr>
                            <td colspan="8" style="text-align: right"><strong>Jumlah :</strong></td>
                            <td><strong><?php echo 'Rp. ' . number_format( $jumlah[0]['jumlah'], 0 , '' , '.' ) . ',-';?></strong></td>
                          </tr>
                        </tbody>
                  </table>
              </div>
